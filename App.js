...
import {ActivityIndicator, Alert, AppState, Linking, Pressable, SafeAreaView, ScrollView, StyleSheet, Text, View, Image} from 'react-native';
...
function NeonIcon({children}){return <LinearGradient colors={['#ff00b8','#8c2cff','#315cff']} style={styles.neonIcon}><Text style={styles.neonIconText}>{children}</Text></LinearGradient>}
...
export default function App(){
 ...
 const openServer=useCallback(async()=>{try{if(await Linking.canOpenURL(SERVER_URI)){await Linking.openURL(SERVER_URI);return true}}catch(e){}return false},[]);
 const play=useCallback(async()=>{
  if(busy)return;
  setBusy(true);setPending(true);setProgress(5);
  try{
   setPhase('cache');await installBundledOrRemoteCache();setProgress(45);
   setPhase('client');
   const hasClient=await Linking.canOpenURL(SERVER_URI);
   if(!hasClient) await prepareClient(v=>setProgress(45+Math.round(Math.max(0,Math.min(1,Number(v)||0))*40)));
   setProgress(90);setPhase('connecting');
   const opened=await openServer();
   if(opened){setProgress(100);setPhase('playing');setPending(false);return;}
   setPhase('install');
   Alert.alert('Instalação do cliente','O Android abriu a instalação do cliente MYTHØS. Conclua a instalação e volte ao launcher.');
  }catch(error){setPhase('error');setPending(false);Alert.alert('MYTHØS Launcher','Não foi possível preparar o jogo.\n\n'+String(error?.message||error||'Erro desconhecido'));}
  finally{setBusy(false)}
 },[busy,openServer]);
 useEffect(()=>{const sub=AppState.addEventListener('change',async next=>{const returning=/inactive|background/.test(state.current);state.current=next;if(returning&&next==='active'&&pending){const ok=await openServer();if(ok){setPending(false);setPhase('playing');setProgress(100)}}});return()=>sub.remove()},[openServer,pending]);
 const status=phase==='cache'?'PREPARANDO CACHE DO GTA':phase==='client'?'PREPARANDO CLIENTE SA-MP 2.11':phase==='connecting'?'CONECTANDO AO MYTHØS RP':phase==='playing'?'JOGO ABERTO':phase==='install'?'FINALIZE A INSTALAÇÃO':phase==='error'?'ERRO AO PREPARAR':'PRONTO PARA JOGAR';
 return <SafeAreaView style={styles.root}><StatusBar style="light"/><LinearGradient colors={['#030007','#0a0018','#02040d']} style={StyleSheet.absoluteFill}/><View style={styles.pink}/><View style={styles.blue}/>
