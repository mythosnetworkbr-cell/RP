...
async function openInstalledClient(){
  try{
    await IntentLauncher.startActivityAsync('android.intent.action.MAIN',{category:'android.intent.category.LAUNCHER',packageName:CLIENT_PACKAGE});
    return true;
  }catch{return false}
}
...
export async function launchClient(serverUri=selectedServerUri){
  ...
  try{await Linking.openURL(selectedServerUri);return true}catch{return false}
}
